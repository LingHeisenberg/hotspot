...
function md5(input) {
  ...
  for (index = 64; index <= input.length; index += 64) {
    md5cycle(state, block(input.substring(index - 64, index)));
  }

  input = input.substring(index - 64);

  const tail = Array(16).fill(0);

  for (index = 0; index < input.length; index += 1) {
    tail[index >> 2] |= input.charCodeAt(index) << (index % 4 << 3);
  }
...
function hex(input) {
  ...
  for (let i = 0; i < input.length; i += 1) {
    for (let j = 0; j < 4; j += 1) {
      output += `0${((input[i] >> (j * 8)) & 255).toString(16)}`.slice(-2);
    }
  }
...
export function hexMD5(input) {
  return hex(md5(unescape(encodeURIComponent(String(input)))));
}
